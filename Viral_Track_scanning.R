...
for (k in List_target_path) {
  ...
  STAR_mapping_command = paste("conda run -n py39 STAR --runThreadN",N_thread,"--genomeDir",Index_genome,"--readFilesIn",k,"--outSAMattributes NH HI AS nM NM XS ",
...
for (k in List_output_path) {
  ...
  SAMtools_indexing_command = paste("conda run -n py39 samtools index",temp_sorted_bam)
  ...
  SAMtools_chromosome_count_command = paste("conda run -n py39 samtools idxstats",temp_sorted_bam,">",temp_chromosome_count_path)
  ...
  foreach(i=rownames(temp_chromosome_count)) %dopar% {
    ...
    temp_export_bam_command = paste("conda run -n py39 samtools view -b",temp_sorted_bam,i,">",paste(k,"Viral_BAM_files/",i,".bam ",sep = ""))
    temp_export_bam_command = paste("conda run -n py39 samtools view -b ",temp_sorted_bam," \'",i,"\'"," > \'",k,"Viral_BAM_files/",i,".bam\'",sep = "")
  ...
  Merging_BAM_commad = paste("conda run -n py39 samtools merge",paste(k,"Merged_viral_mapping.bam",sep = ""),list_BAM_files)
