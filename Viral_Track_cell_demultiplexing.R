...
for (path_temp in List_target_path) {
  ...
  command_merge = base::paste("conda activate py39 && conda run samtools merge ",List_target_path,"/Reads_to_demultiplex.bam -f ",paste(List_bam_files,collapse = " "),sep="")
 ...
 command_sort =paste("conda activate py39 && conda run samtools sort ",List_target_path,"/Reads_to_demultiplex.bam.featureCounts.bam -o ",List_target_path,"/Assigned_sorted.bam",sep = "")
 system(command_sort)
 
 command_index =paste("conda activate py39 && conda run samtools index ",List_target_path,"/Assigned_sorted.bam",sep = "")
 ...
 command_umi_tools = paste("conda activate py39 && conda run umi_tools count --per-gene --gene-tag=XT --assigned-status-tag=XS --per-cell -I ",
