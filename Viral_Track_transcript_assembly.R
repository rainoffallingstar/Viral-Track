...
for (k in Identified_viral_fragments) {
  ...
  for (i in List_target_path) {
    ...
    samtools_extraction_command = paste("conda run -n py39 samtools view -b ",i,name_target,"_Aligned.sortedByCoord.out.bam "
...
for (k in Identified_viral_fragments) {
  ...
  samtools_merging_command = paste("conda run -n py39 samtools merge",paste(temp_path,k,"_merge.bam",sep = ""),
                                   paste(temp_path,"*",sep = ""))
  ...
  samtools_merging_command = paste("conda run -n py39 samtools merge \'",temp_path,k,"_merge.bam\' ",l,sep = "")
...
for (k in Identified_viral_fragments) {
  ...
  stringtie_command = paste("conda run -n py39 stringtie ","\'",temp_path,k,"_merge.bam\'"," -f 0.01 -g 5 ",
